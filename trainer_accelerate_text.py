...
from datasets import EmotionDataset, collate_fn, collate_fn_text, preprocess_audio, EmotionDataset_Text
...
from Wav2Vec2_seq_clf import custom_Wav2Vec2ForEmotionClassification, HybridEmotionModel, custom_Wav2Vec2ForEmotionClassification_Text
...
def create_model_and_processor(freeze_base_model: bool = True, num_speakers: int = 500):
    ...
    model = custom_Wav2Vec2ForEmotionClassification_Text.from_pretrained(
        config.model.model_name,
        config=model_config,
    ...
    try:
        ...
        if accelerator.is_main_process:
            ...
            wandb.init(
                ...
                name="Text adv lambda 0.75 (spk id normal) 20 epoch last 15 Adversary 4 Class Classification Large Dataset Balancing (Multi-GPU) class_weight label_smoothing 0.1",
                config={
        ...
        if config.data.use_large_dataset:
            ...
            # Large 데이터셋 인덱스 생성
            large_index = build_large_corpus_index(str(config.paths.large_data_dir), with_sentence=True)
            if not large_index:
                if accelerator.is_main_process:
        ...
        # 데이터 분할
        if config.data.use_large_dataset:
            (train_paths, train_labels, train_sentences), (val_paths, val_labels, val_sentences), (test_paths, test_labels, test_sentences) = \
                split_large_dataset(
                    index,
        ...
        # 데이터셋 생성
        train_dataset = EmotionDataset_Text(train_paths, train_labels, train_sentences, processor, is_training=True)
        val_dataset = EmotionDataset_Text(val_paths, val_labels, val_sentences, processor, is_training=False)
        test_dataset = EmotionDataset_Text(test_paths, test_labels, test_sentences, processor, is_training=False)
        ...
        try:
            ...
            if accelerator.is_main_process:
                ...
                model_save_path = os.path.join(output_dir, "Text_Adversary_class4_v3_LargeDataset_MultiGPU_batch8_real_adv_lambda_0.75_20_epoch_spk_id_normal_last_15_class_weight_label_smoothing0.1")
